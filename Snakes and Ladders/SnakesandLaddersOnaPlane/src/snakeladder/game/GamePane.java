...
public class GamePane extends GameGrid
{
  ...
  public void reverseAllConnections() {
    for (int i = 0; i < getConnections().size(); i++) {
      int cellStart = getConnections().get(i).cellStart;
      int cellEnd = getConnections().get(i).cellEnd;

      if (getConnections().get(i) instanceof Snake) {
        Ladder newLadder = new Ladder(cellEnd, cellStart);
        getConnections().set(i, newLadder);
      } else if (getConnections().get(i) instanceof Ladder) {
        Snake newSnake = new Snake(cellEnd, cellStart);
        getConnections().set(i, newSnake);
      }
    }
  }

        ...
        implements GGButtonListener
{
  private class SimulatedPlayer extends Thread
  {
    public void run()
    {
      while (true) {
        Monitor.putSleep();
        handBtn.show(1);
        die.get(gp.getCurrentPuppetIndex()).roll(NavigationPane.this, MANUAL_TAG);
        delay(1000);
        handBtn.show(0);
      }
    }
  ...
  private final int MANUAL_TAG = 0;
  ...
  void setupDieValues() {
    gp.rollStatsTracker = new int[numberOfPlayers][numberOfDice*6 + 2];
    for (int i = 0; i < numberOfPlayers; i++){
      for(int j = 0; j < numberOfDice*6 + 2; j++){
        gp.rollStatsTracker[i][j] = 0;
      }
    }
    for (int i = 0; i < numberOfPlayers; i++) {
      ArrayList<Integer> dieNum = new ArrayList<>();
      if (properties.getProperty("die_values." + i) != null) {
        ...
        for (int j = 0; j < dieValueStrings.length; j++) {
          dieNum.add(Integer.parseInt(dieValueStrings[j]));
        }
        dieValues.add(dieNum);
      } else {
  ...
  class ManualDieButton implements GGButtonListener {
    ...
    public void buttonClicked(GGButton ggButton) {
      ...
      if (ggButton instanceof CustomGGButton) {
        ...
        prepareBeforeRoll();
        die.get(gp.getCurrentPuppetIndex()).roll(NavigationPane.this, tag);
      }
    }
  }
  ...
  void prepareRoll(int currentIndex)
  {
    if (currentIndex == 100)  // Game over
    {
      ...
      String countOutput = "";
      for (int i = 0; i < numberOfPlayers; i++){
        countOutput += "Player " + i + ": ";
        for(int j = numberOfDice; j < 6*numberOfDice + 1; j++){
          countOutput += "" + j + "-" + gp.rollStatsTracker[i][j] + " ";
        }
        countOutput += "\n";
        countOutput += "up-" + gp.rollStatsTracker[i][0]
                + " down-" + gp.rollStatsTracker[i][6*numberOfDice + 1] + "\n";
      }
      System.out.print(countOutput);
      ...
      gamePlayCallback.finishGameWithResults(nbRolls % numberOfPlayers, playerPositions);
      gp.resetAllPuppets();
    }
    else
    {
  ...
  void finishRolling(int nb){
    ...
    if(nbRolls % numberOfDice == 0){
      ...
      gp.getPuppet().setActEnabled(false);
    }
  }
  ...
  {
    showStatus("Moving...");
    showScore("# Rolls: " + (nbRolls / numberOfDice));
    gp.rollStatsTracker[gp.getCurrentPuppetIndex()][nb] += 1;
    gp.getPuppet().go(nb);
    totalMove = 0;
  }
